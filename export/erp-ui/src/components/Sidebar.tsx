...
import {
  Home, BarChart3, Leaf, ScanLine, ChevronDown,
  ShoppingCart, Package, Factory, FileBarChart, BookOpen,
  QrCode, Link2, Award, Layers, GitBranch, Search,
  MapPin, Sprout, FlaskConical, Scissors, CloudSun, ClipboardCheck,
  ShoppingBasket, Cpu, Wifi, Activity, Server,
} from "lucide-react";
...
const MODULE_CONFIG = [
  { key: "erp",        href: "/module/erp",        subBase: "/module/erp",        icon: BarChart3,  label: "ERP",                  subItems: ERP_SUB_ITEMS,       color: "text-primary bg-primary/10",   pathPrefix: "/module/erp" },
  { key: "txng",       href: "/module/txng",       subBase: "/module/txng",       icon: ScanLine,   label: "Truy xuất nguồn gốc",  subItems: TXNG_SUB_ITEMS,      color: "text-blue-700 bg-blue-50",     pathPrefix: "/module/txng" },
  { key: "vung_trong", href: "/module/vung-trong", subBase: "/module/vung-trong", icon: Leaf,       label: "Vùng trồng",           subItems: VUNG_TRONG_SUB_ITEMS, color: "text-amber-700 bg-amber-50",  pathPrefix: "/module/vung-trong" },
];
      ...
      <aside className={`fixed left-0 top-16 bottom-0 w-60 bg-white border-r border-border z-20 flex flex-col transition-transform duration-200 lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="px-4 py-5 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">ERP v1.0</p>
            </div>
          </div>
        </div>
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          ...
          {visibleModules.map(m => (
            <ExpandableNavItem
              key={m.key}
              href={m.href} subBase={m.subBase} icon={m.icon} label={m.label}
